...
> this repo's architecture, every dependency, the build/flash flow, and the quality
...
One thing runs on the microcontroller:

1. **A heartbeat LED** that blinks forever — proof that the system is alive and the scheduler runs.

It is written in **`async` Rust** using **Embassy**, a framework that lets you write
cooperative multitasking firmware without a traditional RTOS. The heartbeat is deliberately the
*smallest possible* Embassy task — a clean foundation to build more tasks onto.
...
- **The executor** — Embassy provides an `async` *executor* (scheduler). You write `async fn` tasks;
  the executor runs them, and when a task `.await`s something (e.g. a timer), the CPU is free
...
### The source file (the actual firmware)

#### `src/main.rs` — the entry point and the heartbeat task

This is where the program starts. Its jobs:

1. **Declare the crate-wide rules** at the top (`#![no_std]`, `#![no_main]`, and the strict lints
   like `#![deny(unsafe_code)]`). These apply to *every* file in the crate. Note the crate contains
   **zero `unsafe`** — the `deny` gate has no exceptions anywhere.
2. **`async fn main`** — initialize the hardware (`embassy_stm32::init`), create the LED, then
   **spawn** the `heartbeat` task onto the executor.
...
Today there is one player (the heartbeat); adding more tasks is the natural next step.
...
The task shares the CPU core with the executor. Whenever it `.await`s (the LED timer), the executor
puts the core to sleep until the timer fires. No threads, no locks. Add more `async fn` tasks and
the same executor interleaves them cooperatively.
...
2. Add a second task (e.g. a second LED, or read a button — the latter needs the `exti`
   feature re-enabled in `Cargo.toml` for `embassy-stm32`'s `ExtiInput`).
3. Study how `.await` yields control — add more `defmt::info!` logs to see the interleaving.
4. Once comfortable, layer on a real peripheral (UART, SPI, timer) as its own task.
