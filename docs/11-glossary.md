...
- **`bind_interrupts!`** — Embassy macro that wires hardware interrupt vectors to the HAL's
  handlers. Emits `unsafe`, hence fenced. *See Doc 04/06.*
...
- **`bss` / `data`** — linker sections for zero-initialized / initialized global variables
  (in RAM). *See Doc 03.*
- **CAN (Controller Area Network)** — the robust, prioritized automotive communication bus
  (ISO 11898). *See Doc 06.*
- **CAN FD (Flexible Data-rate)** — CAN with up to 64-byte payloads and a faster data phase.
  *See Doc 06.*
- **Crate** — a Rust library or binary package. *See Doc 08.*
...
- **`defmt`** — "deferred formatting," an ultra-efficient logging framework that sends
  interned IDs, not strings. *See Doc 08.*
- **`defmt-rtt`** — the transport that carries `defmt` frames off-chip over RTT. *See Doc 08.*
- **`deny` / `warn` / `allow`** — lint levels: hard error / warning / permitted. *See Doc 10.*
- **DLC (Data Length Code)** — CAN frame field encoding the payload size. *See Doc 06.*
...
- **Executor** — the async scheduler that polls futures and sleeps the core when idle.
  *See Doc 04.*
- **EXTI** — STM32's external interrupt/event controller (e.g. for button GPIOs). *See Doc 08.*
- **FDCAN** — ST's peripheral implementing CAN FD. This chip has one, `FDCAN1`. *See Doc 06.*
- **FIFO** — First-In-First-Out hardware queue; CAN RX/TX use FIFOs. *See Doc 06.*
- **`flip-link`** — linker wrapper that places the stack so overflow faults instead of
  corrupting data. *See Doc 03/09.*
...
- **ISO 11898** — the CAN standard family. *See Doc 06.*
...
- **Lint** — an automated code-quality check (Clippy/compiler). *See Doc 10.*
...
- **`panic-probe`** — supplies the `#[panic_handler]` that logs then halts. *See Doc 08.*
- **PLL (Phase-Locked Loop)** — clock circuit that multiplies a frequency up (e.g. to
  144 MHz). *See Doc 05.*
- **`probe-rs`** — tool that flashes firmware and streams logs via a debug probe. *See Doc 09.*
...
- **RTT (Real-Time Transfer)** — SEGGER protocol moving bytes over the debug link via a RAM
  buffer; carries `defmt`. *See Doc 08/09.*
- **RustSec** — the community vulnerability database `cargo-deny` audits against. *See Doc 10.*
- **rustfmt / Clippy** — the official formatter / linter. *See Doc 10.*
...
- **SWD (Serial Wire Debug)** — Arm's 2-pin debug protocol used by the ST-LINK. *See Doc 09.*
- **ST-LINK** — the on-board debugger chip on the Nucleo board. *See Doc 05/09.*
...
- **Task** — an `async fn` marked `#[embassy_executor::task]`, scheduled by the executor.
  *See Doc 04.*
- **TDC (Transmitter Delay Compensation)** — CAN FD feature for high data rates. *See Doc 06.*
...
- **Transceiver (CAN)** — chip converting MCU logic TX/RX to differential CAN-H/CAN-L.
  *See Doc 06.*
...
- **`unsafe`** — Rust keyword disabling some compile-time checks; forbidden crate-wide here
  except one fenced spot. *See Doc 10.*
...
- **Wrapping arithmetic (`wrapping_add`)** — arithmetic that intentionally wraps on overflow
  (255→0) instead of panicking. *See Doc 06/08.*
